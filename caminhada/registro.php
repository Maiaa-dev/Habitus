<?php 
    ...
    if (isset($_POST['opcao'])) {
        ...
        $sql = "SELECT rc.meta_cumprida FROM registros_caminhada rc WHERE rc.id_usuario = '$idUsuario' and rc.id_habito = '$idHabito' and DATE(data_feito) = CURDATE()";
        ...
        if ($resultado && mysqli_num_rows($resultado) > 0){
            ...
            if($somaRegistro < $nomeMeta){
                ...
                $sql = "INSERT INTO registros_caminhada (id_usuario,id_habito,id_meta,meta_cumprida) VALUES ('$idUsuario','$idHabito','$idMeta','$metaCumprida')";
                if (mysqli_query($conexao,$sql)){
                    mysqli_query($conexao, "SET FOREIGN_KEY_CHECKS = 1"); //reabilitar a verificação de chave estrangeira
                    exibirAlerta("success", "Registro feito com sucesso!", "Parabéns por cuidar da sua saúde física! Mas, você ainda pode caminhar mais hoje!");
                ...
                exibirAlerta("info", "Muito bom, mas...", "Você já atingiu sua meta de caminhada para hoje, logo, não é necessário registrar mais.");
                header("Location: index.php");
                exit;
            }
    }
